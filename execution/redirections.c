...
/*   Updated: 2023/06/24 03:41:04 by drtaili          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../minishell.h"

void	red_output(t_voidlst *commands, t_voidlst *red,
	t_list_env **m_export, t_list_env **new_env)
{
	int	store_fd;
	int	fd_out;

	fd_out = open(((t_token *)red->content)->str, O_WRONLY
...
void	red_input(t_voidlst *commands, t_voidlst *red,
	t_list_env **m_export, t_list_env **new_env)
{
	int	store_fd;
	int	fd_in;

	fd_in = open(((t_token *)red->content)->str, O_RDONLY, 0644);
	if (fd_in == -1)
	{
		ft_printf(2, "minishell: %s ", ((t_token *)red->content)->str);
		perror("");
...
void	red_double_output(t_voidlst *commands, t_voidlst *red,
	t_list_env **m_export, t_list_env **new_env)
{
	int	store_fd;
	int	d_fd_out;

	d_fd_out = open(((t_token *)red->content)->str, O_WRONLY
...
void	red_double_input(t_voidlst *commands, t_voidlst *red,
	t_list_env **m_export, t_list_env **new_env)
{
	int	store_fd;
	int	fd[2];
	...
	t_list_env **m_export, t_list_env **new_env)
{
	if (red == NULL && ((t_command *)(t_voidlst *)commands->content)->args[0])
		execute_commands(((t_command *)(t_voidlst *)commands->content)->args,
			new_env, m_export);
